...
K_ARTICLES, K_VIDEOS_PER_STEP, MIN_SCORE = 5, 1, 0.50
...
class _Real:
    ...
    def answer(self, history, query):
        ...
        steps = nb.parse_steps(answer)                                          # notebook code
        step_hits = self.videos.retrieve_for_steps(steps, K_VIDEOS_PER_STEP)    # notebook code: one clip per step
        videos = []
        for i, (step, hits) in enumerate(zip(steps, step_hits), 1):
            if not hits:
                continue
            h = hits[0]
            h["rank"], h["step"] = i, step
            h["title"] = h.get("task_name") or step
            videos.append(h)

        return {"answer": answer, "context": context, "articles": articles, "videos": videos,
                "prompt_tokens": self.count_tokens(prompt),
                "latency": {"LLM generation": t_gen1 - t_gen0}}
...
class _Mock:
    ...
    _STEPS = [
        "Prepare safely — Pull over to a flat, stable surface and turn on your hazard lights.",
        "Loosen the lug nuts — Before jacking the car, break the lug nuts loose.",
        "Jack the vehicle — Raise it until the flat tire is off the ground.",
        "Swap the tire — Remove the flat, mount the spare, hand-tighten in a star pattern.",
        "Lower and torque — Lower the vehicle, then fully tighten the lug nuts.",
    ]
    _ANSWER = (
        "Based on the retrieved articles and video clips, here's how to change a car tire:\n\n"
        + "\n".join(f"{i}. {s}" for i, s in enumerate(_STEPS, 1)))
    ...
    def answer(self, history, query):
        ...
        videos = [{"rank": i, "video_id": v, "title": t, "step": step,
                   "start_time": st, "end_time": en, "score": sc}
                  for i, ((v, t, st, en, sc), step) in enumerate(zip(self._VIDEOS, self._STEPS), 1)]
...
class RagBackend:
    def __init__(self):
        ...
        self.settings = {
            "k (articles)": K_ARTICLES, "k (video clips per step)": K_VIDEOS_PER_STEP,
