...
Tasks:
  ...
  3  Video panel: a video player + a ranked playlist of segments; clicking a segment loads
     its clip, seeks to the timestamp and autoplays. Video content is never in the prompt.
...
def render_videos(videos):
    if not videos:
        return "<div class='rhead'>TOP-5 · COSINE SIMILARITY</div><div class='empty'>Ask a question to retrieve video segments.</div>"
    rows = ["<div class='rhead'>TOP-5 · COSINE SIMILARITY</div>"]
    for v in videos:
        p = round(v["score"] * 100)
        cls = pct_cls(p)
...
def render_inspection(articles, videos, prompt_tokens, latency):
    ...
    vid_items = "".join(
        ...
        for v in videos[:2]) or "<div class='inj-empty'>—</div>"
    ...
    for k, v in latency.items():
        ...
        lat.append(f"<div class='kv {'total' if k=='Total' else ''}'><span>{escape(k)}</span>"
    ...
    <div class='insp'>
      ...
      <div class='sub'>Video clips</div>{vid_items}
...
def on_user(message, history):
    """Step 1 (instant): append the user's message and clear the input right away."""
    ...
    with gr.Row(elem_id="mainrow"):
        ...
        with gr.Column(scale=5, elem_id="inspcol", min_width=0):
            ...
            inspection_html = gr.HTML(render_inspection(
                [], [], 0, {"Query encode": 0, "FAISS search": 0, "LLM generation": 0, "Total": 0}))
